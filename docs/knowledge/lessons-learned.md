...
問題: 公開BackendをBlobへ移行した後も、更新履歴生成だけがNeon PostgreSQLに依存していた。

原因: 初回Blob移行では既存DBの履歴をsnapshotへコピーし、次回以降の履歴判定方法を分離していた。

対応: 直前の検証済みsnapshotと新しい公式値を比較し、同一期間の数値変更だけを履歴へ追加する。新規期間は履歴化せず、既存期間の欠落はfail closedとする。

今後: immutable snapshotを正本にする場合、現在値だけでなく差分履歴の生成・再実行・欠落時の安全性も同じartifact系列で検証する。
