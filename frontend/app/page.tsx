...
export default async function Dashboard() {
  ...
  return <main className="page py-12 md:py-20">
    ...
    {indicators.some((indicator) => indicator.developmentData) && <div className="mt-10"><DevelopmentNotice partial={indicators.some((indicator) => !indicator.developmentData)} /></div>}
    <div id="indicators" className="mt-10 scroll-mt-6">
      {indicators.length === 0
        ? <EmptyState description="現在、表示できる指標が登録されていません。" />
        : <section className="grid gap-5 md:grid-cols-2 lg:grid-cols-3" aria-label="主要指標">
